...
export class CreatePlanDto {
  ...
  @ApiProperty({
    example: 299.99,
    description:
      'Monto que se aplicará cada vez que se cobre la suscripción. Debe ser mayor a cero, con hasta 2 dígitos decimales',
  })
  amount: number;
  ...
  @ApiProperty({
    example: 3,
    description:
      'Número de reintentos de cobro de la suscripción. Cuando se agotan los intentos se pone en el estatus determinado por status_after_retry',
    default: 3,
  })
  retryTimes?: number;
  ...
  @ApiProperty({
    example: 'cancelled',
    description:
      'Este campo especifica el status en el que se pondrá la suscripción una vez que se agotaron los intentos',
  ...
  @ApiProperty({
    example: 'active',
    description: 'Estatus del Plan (active = activo, deleted = desactivado)',
    enum: ['active', 'deleted'],
    default: 'active',
  })
  status?: string;
